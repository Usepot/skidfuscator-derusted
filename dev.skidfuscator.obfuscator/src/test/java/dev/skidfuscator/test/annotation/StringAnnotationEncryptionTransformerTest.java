...
import dev.skidfuscator.testclasses.annotation.StringAnnotationTarget;
...
public class StringAnnotationEncryptionTransformerTest {
    private static final String TARGET = "dev/skidfuscator/testclasses/annotation/StringAnnotationTarget";
    private static final String SECRET = "Ldev/skidfuscator/testclasses/annotation/StringAnnotationTarget$Secret;";
    private static final String TYPE_SECRET = "Ldev/skidfuscator/testclasses/annotation/StringAnnotationTarget$TypeSecret;";
    ...
    public void testTransformerIsWiredIntoDefaultList() {
        ...
        assertTrue(skidfuscator.getTransformers()
                .stream()
                .anyMatch(StringAnnotationEncryptionTransformer.class::isInstance));
    }

    @Test
    public void testRemapsStoredStringsAndDecryptsDirectAccessors() throws Exception {
        final AtomicReference<List<Map.Entry<String, byte[]>>> output = new AtomicReference<>();
        final Skidfuscator skidfuscator = new StringAnnotationOnlySkidfuscator(
                new Class<?>[] {
                        StringAnnotationTarget.class,
                        StringAnnotationTarget.Secret.class,
                        StringAnnotationTarget.TypeSecret.class
                },
                output::set
        );
    ...
    private static void assertRemappedAnnotationConstants(final List<Map.Entry<String, byte[]>> classes) {
        ...
        assertNotEquals("class-secret", stringValue(classAnnotation, "value"));
        assertFirstArrayValueRemapped(classAnnotation, "tags", "class-tag-a");
        ...
        assertNotEquals("field-secret", stringValue(fieldAnnotation, "value"));
        assertFirstArrayValueRemapped(fieldAnnotation, "tags", "field-tag");

        final TypeAnnotationNode fieldTypeAnnotation = findTypeAnnotation(field.visibleTypeAnnotations, TYPE_SECRET);
        assertNotNull(fieldTypeAnnotation);
        assertNotEquals("field-type-secret", stringValue(fieldTypeAnnotation, "value"));
        assertFirstArrayValueRemapped(fieldTypeAnnotation, "tags", "field-type-tag");
        ...
        assertNotEquals("method-secret", stringValue(methodAnnotation, "value"));
        assertFirstArrayValueRemapped(methodAnnotation, "tags", "method-tag");

        final TypeAnnotationNode returnTypeAnnotation = findTypeAnnotation(method.visibleTypeAnnotations, TYPE_SECRET);
        assertNotNull(returnTypeAnnotation);
        assertNotEquals("return-type-secret", stringValue(returnTypeAnnotation, "value"));
        assertFirstArrayValueRemapped(returnTypeAnnotation, "tags", "return-type-tag");
        ...
        final MethodNode defaulted = annotation.methods.stream()
                ...
                .findFirst()
                .orElseThrow();
        assertNotEquals("default-secret", defaulted.annotationDefault);
    }
    ...
    private static void executeTransformedTarget(final List<Map.Entry<String, byte[]>> output) throws Exception {
        try (SkidClassLoader classLoader = new SkidClassLoader(new URL[0])) {
            ...
            final Class<?> clazz = classLoader.loadClass(StringAnnotationTarget.class.getName());
    ...
    private static TypeAnnotationNode findTypeAnnotation(final List<TypeAnnotationNode> annotations, final String descriptor) {
        if (annotations == null) {
            return null;
        }
        return annotations.stream()
    ...
    private static String stringValue(final AnnotationNode annotation, final String name) {
    ...
    private static void assertFirstArrayValueRemapped(final AnnotationNode annotation,
                                                     final String name,
                                                     final String originalValue) {
        for (int i = 0; i < annotation.values.size() - 1; i += 2) {
            if (name.equals(annotation.values.get(i))) {
                final List<?> values = (List<?>) annotation.values.get(i + 1);
                assertNotNull(values);
                assertTrue(!values.isEmpty());
                assertNotEquals(originalValue, values.get(0));
                return;
            }
        }
        throw new IllegalStateException("Missing annotation value " + name);
    ...
    private static final class StringAnnotationOnlySkidfuscator extends TestSkidfuscator {
        private StringAnnotationOnlySkidfuscator(final Class<?>[] test,
        ...
        public List<Transformer> getTransformers() {
            return Collections.singletonList(new StringAnnotationEncryptionTransformer(this));
        }
    }
    ...
    private static final class ConfigOnlySkidfuscator extends Skidfuscator {
        private ConfigOnlySkidfuscator() {
            super(SkidfuscatorSession.builder()
                    .config(new File(StringAnnotationEncryptionTransformerTest.class
