...
public class BytesClinitV3EncryptionGenerator extends AbstractEncryptionGeneratorV3 {
    ...
    private static String decryptMeBitch(final byte[] input, final int maskedSeed, final int site, final int salt) {
        int context = 0x6D2B79F5;
        for (byte value : localKeys) {
            context ^= value & 0xFF;
            context *= 0x45D9F3B;
            context ^= context >>> 16;
        }
    ...
    private static String decryptMeBitchWide(final byte[] input, final long maskedSeed, final int site, final int salt) {
        int context = 0x6D2B79F5;
        for (byte value : localKeys) {
            context ^= value & 0xFF;
            context *= 0x45D9F3B;
